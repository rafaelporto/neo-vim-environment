...
require("mason-lspconfig").setup({
    ...
    -- for each installed server using filetypes/cmd from nvim-lspconfig's lsp/*.lua files.
})
...
for _, server in ipairs({ "ts_ls", "eslint", "clojure_lsp", "gopls", "cssls", "marksman", "dockerls", "docker_compose_language_service" }) do
...
vim.lsp.config["lua_ls"] = {
    ...
    on_init = function(client)
        ...
        if path and (
            path:find(vim.fn.stdpath("config"), 1, true) or
            path:find(vim.fn.stdpath("data"), 1, true)
        ) then
...
-- omnisharp — custom definition handler + per-server keymap
vim.lsp.config["omnisharp"] = {
    ...
    on_attach = function(_, bufnr)
        vim.keymap.set("n", "<leader>gdr",
            ...
            { buffer = bufnr, desc = "OmniSharp: Telescope Definitions" })
    end,
}
...
vim.api.nvim_create_autocmd("LspAttach", {
    ...
    callback = function(ev)
        ...
        vim.keymap.set("n", "<leader>rl", ":LspRestart | LspStart<CR>",
            { buffer = bufnr, remap = true, desc = "Restart LSP" })

        -- Goto
        vim.keymap.set("n", "gd", function() vim.lsp.buf.definition() end,
            { buffer = bufnr, remap = true, desc = "Go to Definition" })
        vim.keymap.set("n", "gi", function() vim.lsp.buf.implementation() end, opts)
        vim.keymap.set("n", "gr", function() vim.lsp.buf.references() end,
            { buffer = bufnr, remap = true, desc = "References" })

        -- LSP actions
        vim.keymap.set("n", "K",           function() vim.lsp.buf.hover() end,
            { buffer = bufnr, remap = true, desc = "Hover" })
        vim.keymap.set("n", "<leader>vds", function() vim.lsp.buf.document_symbol() end, opts)
        vim.keymap.set("n", "<leader>vws", function() vim.lsp.buf.workspace_symbol() end, opts)
        vim.keymap.set("n", "<leader>rn",  function() vim.lsp.buf.rename() end,
            { buffer = bufnr, remap = true, desc = "Rename" })
        ...
        vim.keymap.set("n", "<leader>aa", function() vim.diagnostic.setqflist() end,
            { buffer = bufnr, remap = false, desc = "All diagnostics in quickfix" })
        vim.keymap.set("n", "<leader>ae", function() vim.diagnostic.setqflist({ severity = "E" }) end,
            { buffer = bufnr, remap = false, desc = "All workspace errors in quickfix" })
        vim.keymap.set("n", "<leader>aw", function() vim.diagnostic.setqflist({ severity = "W" }) end,
            { buffer = bufnr, remap = false, desc = "All workspace warnings in quickfix" })
        vim.keymap.set("n", "<leader>d",  function() vim.diagnostic.setloclist() end,
            { buffer = bufnr, remap = false, desc = "Buffer diagnostics in loclist" })
    end,
})
